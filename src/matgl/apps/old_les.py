...
class EwaldPotential(nn.Module):
    ...
    def forward(self, g, lattice, **kwargs):
        if "batch" not in g.ndata:
            n_nodes = g.ndata['pos'].shape[0]
            batch_now = torch.zeros(n_nodes, dtype=torch.int64, device=g.ndata['pos'].device)
        else:
        ...
        box = lattice # .view(-1, 3, 3).diagonal(dim1=-2, dim2=-1)
        r = g.ndata['pos']
        q = g.ndata[self.feature_key]
        if q.dim() == 1:
            q = q.unsqueeze(1)
        ...
        for i in unique_batches:
            ...
            if self.exponent == 1 and hasattr(self, 'external_field') and self.external_field is not None:
                ...
                if isinstance(self.external_field_direction, int):
                    ...
                    try:
                        direction_index_now = int(g.ndata[self.external_field_direction][i])
                    except:
                ...
                if isinstance(self.external_field, float):
                    ...
                    try:
                        external_field_now = g.ndata[self.external_field][i]
                    except:
        ...
        # g.ndata[self.output_key] = 
        return_value = torch.stack(results, dim=0).sum(axis=1) if self.aggregation_mode == "sum" else torch.stack(results, dim=0)

        if self.compute_field:
            return_value = {self.output_key: return_value, self.feature_key+'_field': torch.cat(field_results, dim=0)}
        else:
            return_value = {self.output_key: return_value}
        return return_value
